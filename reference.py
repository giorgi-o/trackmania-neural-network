...
def optimize_model():
    ...
    # [[0.05], [0.05], ...] w/ grad_fn
    # => the q value of the best action
    ...
    # [0.05, 0.07, ...]
    # => the q value of the best action (next_state)

    # Compute the expected Q values
    expected_state_action_values = reward_batch + (next_state_values * GAMMA)
    # [-0.95, 0.93, ...]
    # .unsqueeze(1) => [[-0.95], [0.93], ...]
    # => the td_targets
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        (x_axis_position, velocity) = observation
        reward += abs(velocity)  # * 100

        print(
            f"Step {t}: Action {action}, Reward {reward}, Done {terminated or truncated}"
        )
